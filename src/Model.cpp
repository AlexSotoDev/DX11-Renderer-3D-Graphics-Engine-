...
Model::Model(ID3D11Device* dev, Model::PreMadedeModels pm, float size)
{
	mDevice = dev;
	pStdVerts = nullptr;
	pTriList = nullptr;
	...
	case UnitPyramidRepeatedTexture:
		...
		break;
	case UnitSkyBoxSixFaceTexture:
		ModelTools::CreateSkyBoxSixFaceTexture(pStdVerts, numVerts, pTriList, numTris, size);
		break;
		//case UnitPyramid:
		//	ModelTools::CreateUnitPyramid(pStdVerts, numVerts, pTriList, numTris);
		//	break;
	case UnitSphere:
		ModelTools::CreateUnitSphere(size, size, pStdVerts, numVerts, pTriList, numTris);
		break;
	default:
